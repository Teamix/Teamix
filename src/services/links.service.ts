...
export class LinksService {
  ...
  constructor(public itemsPlaylist: ItemsPlaylistService, public http: HttpClient) { this.fetchLinks(); }
  ...
  public fetchLinks() {
    return this.http.get('https://firebasestorage.googleapis.com/v0/b/teamixwebsite.appspot.com/o/Jsons%2FfirebaseLinks.json?alt=media&token=98c1f18a-ea6e-4828-a4cd-a082f2655775').subscribe(
      res => {
        this.numOfLinks = [];
        const check = Object.keys(res);
        for (const item of check) {
          this.numOfLinks.push(res[item]);
          this.numOfLinks[item].playlist = this.itemsPlaylist.getItemsPlaylist()[this.numOfLinks[item].playlist];
        }
